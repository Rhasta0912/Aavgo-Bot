...
async function buildAdminHoursSnapshot(client) {
  ...
  for (const row of rows) {
    ...
    const person = {
      ...
      todayHoursLabel: activeNow ? 'IN SHIFT' : formatHours(dayHistory.totalHours),
      weeklyHours: roundHours(totals.weeklyHours),
      weeklyHoursLabel: activeNow ? 'IN SHIFT' : formatHours(totals.weeklyHours),
      monthlyHours: roundHours(totals.monthlyHours),
      monthlyHoursLabel: activeNow ? 'IN SHIFT' : formatHours(totals.monthlyHours),
      finalizedAllHours: roundHours(totals.allHours),
      allHours: activeNow ? 'IN SHIFT' : roundHours(totals.allHours),
      allHoursLabel: activeNow ? 'IN SHIFT' : formatHours(totals.allHours),
      payPeriods,
      currentMonth: {
